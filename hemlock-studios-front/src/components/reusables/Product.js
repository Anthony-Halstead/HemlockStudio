...
const handleRemoveMessage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://3.16.219.108:8080/user/findFavoriteProducts", {
        headers: {
  ...
  const handleAddToCartClick = (event) => {
    ...
    axios.post("https://3.16.219.108:8080/cart/addItemToCart", { productId: product.id },{
        headers: {
          'Authorization': `Bearer ${jwtToken}`
        }
      })
  ...
  const handleFavoritedClick = (event) => {
    ...
    if (isFavorited) {
      axios.delete(`https://3.16.219.108:8080/user/removeProductFromFavorites/${favoriteDeleteData}`,{
        headers: {
          'Authorization': `Bearer ${jwtToken}`
        }
      })
      ...
      axios.post("https://3.16.219.108:8080/user/addProductToFavorites", { productId: product.id },{
        headers: {
          'Authorization': `Bearer ${jwtToken}`
        }
      })
