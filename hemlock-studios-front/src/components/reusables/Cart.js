...
const getCartTotal = () => {
  let jwtToken = localStorage.getItem("token");
    ...
    axios.get("https://LBtest-01-1681136195.us-east-2.elb.amazonaws.com/cart/total", {
      headers: {
  ...
  const handleRemoveFromCartClick = (productId) => {
    ...
    axios.delete(`https://LBtest-01-1681136195.us-east-2.elb.amazonaws.com/cart/removeItemFromCart/${productId}`, {
      headers: {
        'Authorization': `Bearer ${jwtToken}`
      }
    })
  ...
  const findProductsInCart = () => {
    let jwtToken = localStorage.getItem("token");
    axios
      .get("https://LBtest-01-1681136195.us-east-2.elb.amazonaws.com/cart/findItemsInCart", {
        headers: {
          'Authorization': `Bearer ${jwtToken}`
        }
      })
