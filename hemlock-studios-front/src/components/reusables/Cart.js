...
const getCartTotal = () => {
  let jwtToken = localStorage.getItem("token");
    ...
    axios.get("http://18.220.71.177:8080/cart/total", {
      headers: {
  ...
  const handleRemoveFromCartClick = (productId) => {
    ...
    axios.delete(`http://18.220.71.177:8080/cart/removeItemFromCart/${productId}`, {
      headers: {
        'Authorization': `Bearer ${jwtToken}`
      }
    })
  ...
  const findProductsInCart = () => {
    let jwtToken = localStorage.getItem("token");
    axios
      .get("http://18.220.71.177:8080/cart/findItemsInCart", {
        headers: {
          'Authorization': `Bearer ${jwtToken}`
        }
      })
