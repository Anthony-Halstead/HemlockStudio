...
function Checkout() {
  ...
  const handleSubmit = () => {
    ...
    axios.post('https://3.16.219.108:8080/cart/makePurchase',{},
        {
          headers: {
  ...
  const handleCouponCode = debounce(() => {
    if (couponCode.trim() !== '') {
      ...
      axios
        .post(
          'https://3.16.219.108:8080/coupon/applyCouponDiscount',
  ...
  const getCartTotal = () => {
    let jwtToken = localStorage.getItem('token');
    axios
      .get('https://3.16.219.108:8080/cart/total', {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
        },
      })
  ...
  const getDiscountedCartTotal = () => {
    let jwtToken = localStorage.getItem('token');
    axios
      .get('https://3.16.219.108:8080/cart/discountedTotal', {
        headers: {
          Authorization: `Bearer ${jwtToken}`,
        },
      })
